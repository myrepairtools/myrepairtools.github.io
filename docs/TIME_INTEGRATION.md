...
| 3 | **Overtime + pace + alerts** | B + C + schedules | **Reporting half shipped:** `report-overtime.html` (new **Reports** rail area / library page) → "Overtime & pace" report: weekly hours vs the 40h line + 5-day projected pace, store-grouped, RLS-scoped. OT rule = 40/wk, **Sun–Sat**. **Alerts half pending Foundation C** (push pending-OT to Teams/SMS). |
