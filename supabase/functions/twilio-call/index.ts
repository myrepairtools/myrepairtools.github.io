...
async function actionCall(payload: any, sentBy: { id?: string; name?: string }) {
  ...
  // path the ready-for-pickup TEXT note uses — the browser could never win the
  // race against the status-change page reload). AWAITED and reported, so the
  // caller can fall back to its own write; a note failure never fails the call.
  ...
  return ok
    ? json({ ok: true, sid, to, from, store, note_ok: noteOk, note_error: noteErr })
...
// "✓ Call placed", the telemetry logged "(server note)", and the ticket had no
