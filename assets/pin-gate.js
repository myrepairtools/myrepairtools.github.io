...
(function () {
  ...
  var PAGEACC = {
    ...
    'schedule-admin.html':'schedule.admin',
  ...
  host.id = 'cpr-pingate';
  host.setAttribute('style', 'position:fixed;inset:0;z-index:2147483646;background:#2D2D3B;display:flex;align-items:center;justify-content:center;font-family:var(--font-sans);padding:24px');
  host.innerHTML = '<div style="color:rgba(255,255,255,.45);font-weight:500;font-family:var(--font-sans)">…</div>';
  ...
  function logoSvg(w, mb){
    return '<svg viewBox="0 0 308 64" width="'+w+'" style="max-width:100%;height:auto;display:block;margin:0 auto '+mb+'px" xmlns="http://www.w3.org/2000/svg" fill="none" role="img" aria-label="myRepairTools">'
      ...
      + '<text x="74" y="44" font-size="30" font-weight="800"><tspan fill="#FFFFFF">myRepair</tspan><tspan fill="#DC282E">Tools</tspan></text>'
      + '</svg>';
  }
  ...
  function gateForm(msg){
    host.innerHTML = ''
      ...
      +   logoSvg(236, 28)
      +   '<input id="cpr-pg-pin" type="password" inputmode="numeric" autocomplete="off" placeholder="Enter PIN" style="width:100%;font-family:var(--font-sans);font-size:1.05rem;text-align:center;letter-spacing:4px;padding:13px;border:1.5px solid rgba(255,255,255,.18);border-radius:11px;background:rgba(255,255,255,.06);color:#fff;outline:none">'
      +   '<button id="cpr-pg-go" style="width:100%;font-family:var(--font-sans);font-weight:600;font-size:.95rem;border:none;border-radius:11px;padding:13px;margin-top:11px;background:#DC282E;color:#fff;cursor:pointer">Sign in</button>'
      +   '<div id="cpr-pg-err" style="color:#F7A6A8;font-size:.78rem;font-weight:500;margin-top:11px;min-height:1em">'+(msg||'')+'</div>'
      + '</div>';
  ...
  function noAccess(name){
    host.innerHTML = ''
      ...
      +   logoSvg(196, 24)
      +   '<div style="font-family:var(--font-sans);font-weight:700;font-size:1.05rem">No access to this tool</div>'
      ...
      +   '<button id="cpr-pg-home" style="font-family:var(--font-sans);font-weight:600;font-size:.82rem;border:none;border-radius:9px;padding:10px 16px;background:#DC282E;color:#fff;cursor:pointer;margin-right:8px">Home</button>'
      +   '<button id="cpr-pg-switch" style="font-family:var(--font-sans);font-weight:600;font-size:.82rem;border:1.5px solid rgba(255,255,255,.22);border-radius:9px;padding:10px 16px;background:transparent;color:#fff;cursor:pointer">Switch user</button>'
      + '</div>';
